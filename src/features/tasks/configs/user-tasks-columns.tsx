...
export const userTasksColumns = (
	...
	},
	{
		...
		cell: ({ row }) => {
			return (
				<div className="w-52 truncate md:w-full lg:min-w-72">
					{row.original.title}
				</div>
			);
		},
	...
	},
	{
		...
		cell: ({ row }) => {
			const fullLink = `/org/${row.original.organizationId}/dashboard`;

			return (
				<Link
					href={fullLink}
					onClick={e => e.stopPropagation()}
					className="flex items-center gap-1 truncate transition-colors hover:text-accent-foreground"
				>
					<OrgAvatar
						size="xs"
						imageUrl={row.original.organization.image}
						name={row.original.organization.name}
					/>
					<span className="line-clamp-1">{row.original.organization.name}</span>
				</Link>
			);
		},
	},
	{
		...
		cell: ({ row }) => {
			...
			const fullLink = `/org/${orgId}/projects/${projectId}`;

			return (
				<Link
					href={fullLink}
					onClick={e => e.stopPropagation()}
					className="truncate transition-colors hover:text-accent-foreground"
				>
