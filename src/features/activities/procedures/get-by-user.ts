...
export const getByUser = protectedProcedure
	...
	.query(async ({ ctx, input }) => {
		...
		const conditions = [eq(activities.authorId, targetUserId)];

		if (input.projectId) {
			conditions.push(eq(activities.projectId, input.projectId));
		}

		if (input.taskId) {
			conditions.push(eq(activities.taskId, input.taskId));
		}
		...
		const data = await db
			...
			.leftJoin(
				sql`projects ep`,
				sql`ep.id = ${activities.entityId} and ${activities.entityType} = 'project'`,
			)
			.leftJoin(
				sql`tasks et`,
				sql`et.id = ${activities.entityId} and ${activities.entityType} = 'task'`,
			)
			.leftJoin(
				sql`subtasks es`,
				sql`es.id = ${activities.entityId} and ${activities.entityType} = 'subtask'`,
			)
			.leftJoin(
				sql`"user" eu`,
				sql`eu.id = ${activities.entityId} and ${activities.entityType} = 'user'`,
			)
		...
		return {
			items: items.map(item => {
				...
				if (item.entityProject.id) {
					entity = item.entityProject;
				} else if (item.entityTask.id) {
					entity = item.entityTask;
				} else if (item.entitySubtask.id) {
					entity = item.entitySubtask;
				} else if (item.entityUser.id) {
					entity = item.entityUser;
				}
